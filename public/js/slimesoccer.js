...
(function(){
  ...
  if(socket){
    socket.on('cli key Up', function(event){
      Key.onKeyUp(event);
    });

    socket.on('cli key Down', function(event){
      Key.onKeyDown(event);
    });
  }
  ...
  SlimeSoccer = function(id){
    ...
    this.ctx = this.canvas.getContext('2d');
	  window.addEventListener('keyup', function(event){Key.onKeyUp(event); if(socket) socket.emit('key Up', {keyCode: event.keyCode});}, false);
	  window.addEventListener('keydown', function(event){Key.onKeyDown(event); if(socket) socket.emit('key Down', {keyCode : event.keyCode});}, false);
